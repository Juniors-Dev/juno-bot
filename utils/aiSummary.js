...
async function summarizeMessages(messages, interaction) {
    ...
    const formattedMessages = messages.map(parseMessage).filter(content => content.length > 0).join('\n');
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        const chunkSummary = await summarizeText(chunks[i], 'Summarize the following chat messages in a concise paragraph:');
    ...
    const finalSummary = await summarizeText(combinedSummaries, 'Combine these summaries into a single, cohesive, final summary of the entire conversation:', 1000);
